...
- (void)fetchWorkoutById:(HKSampleType *)type
            ...
            if (type == [HKObjectType workoutType]) {
              for (HKWorkout *sample in results) {
                @try {
                  ...
                  NSLog(@"RNHealth: An error occured while trying to add "
                        @"sample from: %@ ",
  ...
  HKSampleQuery *query =
      [[HKSampleQuery alloc] initWithSampleType:type
                                      predicate:predicate
                                          limit:lim
...
- (void)fetchWorkoutRoute:(HKSampleType *)type
  ...
  handlerBlock = ^(HKAnchoredObjectQuery *query,
    ...
    if (completion) {
      ...
      for (HKWorkoutRoute *routeSample in sampleObjects) {
        ...
        void (^locationsHandlerBlock)(HKWorkoutRouteQuery *query,
                                      NSArray<CLLocation *> *routeData,
                                      BOOL done, NSError *error);

        locationsHandlerBlock = ^(HKWorkoutRouteQuery *query,
                                  NSArray<CLLocation *> *routeData, BOOL done,
                                  NSError *error) {
          if (!routeData) {
          ...
          for (CLLocation *sample in routeData) {
            @try {
              ...
              NSString *timestamp = [RCTAppleHealthKit
                  buildISO8601StringFromDate:sample.timestamp];
              ...
              NSLog(@"RNHealth: An error occured while trying to add route "
                    @"sample from: %@ ",
                    [[[routeSample sourceRevision] source] bundleIdentifier]);
            }
          }
          ...
          if (done) {
            ...
            NSData *anchorData =
                [NSKeyedArchiver archivedDataWithRootObject:newAnchor];
            NSString *anchorString =
                [anchorData base64EncodedStringWithOptions:0];
            NSString *start = [RCTAppleHealthKit
                buildISO8601StringFromDate:routeSample.startDate];
            NSString *end = [RCTAppleHealthKit
                buildISO8601StringFromDate:routeSample.endDate];
            ...
            NSObject *metaData =
                [routeSample metadata] ? [routeSample metadata] : @{};
        ...
        HKWorkoutRouteQuery *routeQuery =
            [[HKWorkoutRouteQuery alloc] initWithRoute:routeSample
  ...
  HKAnchoredObjectQuery *query =
      [[HKAnchoredObjectQuery alloc] initWithType:type
                                        predicate:predicate
                                           anchor:anchor
...
- (void)fetchMostRecentQuantitySampleOfType:(HKQuantityType *)quantityType
                                  predicate:(NSPredicate *)predicate
                                 completion:(void (^)(HKQuantity *, NSDate *,
                                                      NSDate *,
                                                      NSError *))completion {
      ...
      initWithSampleType:quantityType
         ...
         sortDescriptors:@[ timeSortDescriptor ]
          resultsHandler:^(HKSampleQuery *query, NSArray *results,
            ...
            if (completion) {
              // If quantity isn't in the database, return nil in the completion
              // block.
...
- (void)fetchQuantitySamplesOfType:(HKQuantityType *)quantityType
      ...
      dispatch_async(
          dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            for (HKQuantitySample *sample in results) {
              ...
              NSString *startDateString = [RCTAppleHealthKit
                  buildISO8601StringFromDate:sample.startDate];
              NSString *endDateString =
                  [RCTAppleHealthKit buildISO8601StringFromDate:sample.endDate];
  ...
  HKSampleQuery *query =
      [[HKSampleQuery alloc] initWithSampleType:quantityType
                                      predicate:predicate
                                          limit:lim
                                sortDescriptors:@[ timeSortDescriptor ]
                                 resultsHandler:handlerBlock];
      ...
      dispatch_async(
          dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            if (type == [HKObjectType workoutType]) {
              for (HKWorkout *sample in results) {
                @try {
                  double energy = [[sample totalEnergyBurned]
                      doubleValueForUnit:[HKUnit kilocalorieUnit]];
                  double distance = [[sample totalDistance]
                      doubleValueForUnit:[HKUnit mileUnit]];
                  NSString *type = [RCTAppleHealthKit
                      stringForHKWorkoutActivityType:[sample
                                                         workoutActivityType]];

                  NSString *startDateString = [RCTAppleHealthKit
                      buildISO8601StringFromDate:sample.startDate];
                  NSString *endDateString = [RCTAppleHealthKit
                      buildISO8601StringFromDate:sample.endDate];
                  ...
                  NSDictionary *elem = @{
                    @"activityId" :
                        [NSNumber numberWithInt:[sample workoutActivityType]],
                    @"id" : [[sample UUID] UUIDString],
                    @"activityName" : type,
                    @"calories" : @(energy),
                    @"tracked" : @(isTracked),
                    @"metadata" : [sample metadata] ? [sample metadata]
                                                    : [NSNull null],
                    @"sourceName" : [[[sample sourceRevision] source] name],
                    @"sourceId" :
                        [[[sample sourceRevision] source] bundleIdentifier],
      ...
      dispatch_async(
          dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            for (HKClinicalRecord *record in results) {
              NSString *startDateString = [RCTAppleHealthKit
                  buildISO8601StringFromDate:record.startDate];
              NSString *endDateString =
                  [RCTAppleHealthKit buildISO8601StringFromDate:record.endDate];

              NSError *jsonE = nil;
              NSArray *fhirData = [NSJSONSerialization
                  JSONObjectWithData:record.FHIRResource.data
              ...
              if (@available(iOS 14.0, *)) {
                HKFHIRVersion *fhirResourceVersion =
                    record.FHIRResource.FHIRVersion;
            ...
            for (HKWorkout *sample in sampleObjects) {
              @try {
                ...
                NSString *type = [RCTAppleHealthKit
                    stringForHKWorkoutActivityType:[sample
                                                       workoutActivityType]];
                NSArray *workoutEvents = [RCTAppleHealthKit
                    formatWorkoutEvents:[sample workoutEvents]];
                ...
                bool isTracked = true;
                if ([[sample metadata][HKMetadataKeyWasUserEntered] intValue] ==
                    1) {
                  isTracked = false;
                }
                ...
                NSLog(@"RNHealth: An error occured while trying to add workout "
                      @"sample from: %@ ",
                      [[[sample sourceRevision] source] bundleIdentifier]);
              }
            }
...
- (void)fetchSleepCategorySamplesForPredicate:(NSPredicate *)predicate
      ...
      dispatch_async(
          dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            for (HKCategorySample *sample in results) {
              ...
              // watchOS 9 and iOS 16 introduce Core, Deep, REM, and Awake
              // phases of sleep.
  ...
  HKCategoryType *categoryType = [HKObjectType
      categoryTypeForIdentifier:HKCategoryTypeIdentifierSleepAnalysis];

  HKSampleQuery *query =
      [[HKSampleQuery alloc] initWithSampleType:categoryType
                                      predicate:predicate
                                          limit:lim
                                sortDescriptors:@[ timeSortDescriptor ]
                                 resultsHandler:handlerBlock];
...
- (void)fetchSumOfSamplesTodayForType:(HKQuantityType *)quantityType
                                 unit:(HKUnit *)unit
                           completion:
                               (void (^)(double, NSError *))completionHandler {
  ...
  HKStatisticsQuery *query = [[HKStatisticsQuery alloc]
      ...
      quantitySamplePredicate:predicate
            ...
            completionHandler:^(HKStatisticsQuery *query, HKStatistics *result,
                                NSError *error) {
...
- (void)fetchSumOfSamplesOnDayForType:(HKQuantityType *)quantityType
                 ...
                 includeManuallyAdded:(BOOL)includeManuallyAdded
                                  day:(NSDate *)day
                           completion:(void (^)(double, NSDate *, NSDate *,
                                                NSError *))completionHandler {
  NSPredicate *dayPredicate = [RCTAppleHealthKit predicateForSamplesOnDay:day];
  NSPredicate *predicate =
      [NSCompoundPredicate andPredicateWithSubpredicates:@[ dayPredicate ]];
  if (includeManuallyAdded == false) {
    NSPredicate *manualDataPredicate = [NSPredicate
        predicateWithFormat:@"metadata.%K != YES", HKMetadataKeyWasUserEntered];
    predicate = [NSCompoundPredicate
        andPredicateWithSubpredicates:@[ dayPredicate, manualDataPredicate ]];
  }
  HKStatisticsQuery *query = [[HKStatisticsQuery alloc]
         initWithQuantityType:quantityType
      quantitySamplePredicate:predicate
            ...
            completionHandler:^(HKStatisticsQuery *query, HKStatistics *result,
              ...
              if ([error.localizedDescription
                ...
                if (startDate == nil || endDate == nil) {
                  error = [[NSError alloc]
                      initWithDomain:@"AppleHealthKit"
                                code:0
                            userInfo:@{
                              @"Error reason" :
                                  @"Could not fetch statistics: Not authorized"
                            }];
                }
...
- (void)fetchCumulativeSumStatisticsCollection:(HKQuantityType *)quantityType
                                    ...
                                    completion:(void (^)(NSArray *, NSError *))
                                                   completionHandler {
  ...
  NSDateComponents *anchorComponents = [calendar
      components:NSCalendarUnitDay | NSCalendarUnitMonth | NSCalendarUnitYear
        fromDate:[NSDate date]];
  anchorComponents.hour = 0;
  NSDate *anchorDate = [calendar dateFromComponents:anchorComponents];
  NSPredicate *predicate = [NSPredicate
      predicateWithFormat:@"metadata.%K != YES AND %K >= %@ AND %K <= %@",
    ...
    [results enumerateStatisticsFromDate:startDate
                               ...
                               withBlock:^(HKStatistics *result, BOOL *stop) {
                                 ...
                                 if (quantity) {
                                   ...
                                   NSString *dateString = [RCTAppleHealthKit
                                       buildISO8601StringFromDate:date];
        ...
        enumerateStatisticsFromDate:startDate
                          ...
                          withBlock:^(HKStatistics *result, BOOL *stop) {
                            ...
                            if (quantity) {
                              ...
                              NSString *startDateString = [RCTAppleHealthKit
                                  buildISO8601StringFromDate:startDate];
                              NSString *endDateString = [RCTAppleHealthKit
                                  buildISO8601StringFromDate:endDate];
...
- (void)fetchCumulativeSumStatisticsCollection:(HKQuantityType *)quantityType
  ...
  if (includeManuallyAdded == false) {
    ...
    predicate = [NSPredicate
        predicateWithFormat:@"%K >= %@ AND %K <= %@", HKPredicateKeyPathEndDate,
                            startDate, HKPredicateKeyPathStartDate, endDate];
  }
  // Create the query
  HKStatisticsCollectionQuery *query = [[HKStatisticsCollectionQuery alloc]
         initWithQuantityType:quantityType
                          ...
                          withBlock:^(HKStatistics *result, BOOL *stop) {
                            ...
                            if (quantity) {
                              ...
                              for (HKSource *source in result.sources) {
                                ...
                                HKQuantity *sourceQuantity =
                                    [result sumQuantityForSource:source];
                                double quantity =
                                    [sourceQuantity doubleValueForUnit:unit];
...
- (void)fetchWorkoutForPredicate:(NSPredicate *)predicate
  ...
  void (^handlerBlock)(HKSampleQuery *query, NSArray *results, NSError *error);
  NSSortDescriptor *endDateSortDescriptor =
      [[NSSortDescriptor alloc] initWithKey:HKSampleSortIdentifierEndDate
                                  ascending:ascending];
  handlerBlock = ^(HKSampleQuery *query, NSArray *results, NSError *error) {
    if (!results) {
    ...
    if (completion) {
      NSMutableArray *data = [NSMutableArray arrayWithCapacity:1];
      NSDictionary *numberToWorkoutNameDictionary =
          [RCTAppleHealthKit getNumberToWorkoutNameDictionary];
          ...
          dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            for (HKWorkout *sample in results) {
              double energy = [[sample totalEnergyBurned]
                  doubleValueForUnit:[HKUnit kilocalorieUnit]];
              double distance =
                  [[sample totalDistance] doubleValueForUnit:[HKUnit mileUnit]];
              NSNumber *activityNumber =
                  [NSNumber numberWithInt:[sample workoutActivityType]];

              NSString *activityName =
                  [numberToWorkoutNameDictionary objectForKey:activityNumber];
              ...
              if (activityName) {
                NSDictionary *elem = @{
                  ...
                  @"calories" : @(energy),
                  @"distance" : @(distance),
                  @"startDate" : [RCTAppleHealthKit
                      buildISO8601StringFromDate:sample.startDate],
                  @"endDate" : [RCTAppleHealthKit
                      buildISO8601StringFromDate:sample.endDate]
                };
                [data addObject:elem];
              }
  ...
  HKSampleQuery *query =
      [[HKSampleQuery alloc] initWithSampleType:[HKObjectType workoutType]
                                      predicate:predicate
                                          limit:limit
...
/*!
    Set background observer for the given HealthKit sample type. This method
   should only be called by the native code and not injected by any Javascript
   code, as that might imply in unstable behavior

    @deprecated The setObserver() method has been deprecated in favor of
   initializeBackgroundObservers()
  ...
  HKObserverQuery *query = [[HKObserverQuery alloc]
      initWithSampleType:sampleType
           ...
           updateHandler:^(HKObserverQuery *query,
             ...
             NSLog(@"[HealthKit] New sample received from Apple HealthKit - %@",
                   type);

             NSString *successEvent =
                 [NSString stringWithFormat:@"healthKit:%@:sample", type];

             if (error) {
               completionHandler();

               NSLog(@"[HealthKit] An error happened when receiving a new "
                     @"sample - %@",
                     error.localizedDescription);
             ...
             NSLog(@"[HealthKit] New sample from Apple HealthKit processed "
                   @"(dep) - %@ %@",
                   type, successEvent);
           }];

  [self.healthStore
      enableBackgroundDeliveryForType:sampleType
                            frequency:HKUpdateFrequencyImmediate
                       withCompletion:^(BOOL success,
                                        NSError *_Nullable error) {
                         NSString *successEvent = [NSString
                             stringWithFormat:@"healthKit:%@:enabled", type];

                         if (error) {
                           NSLog(@"[HealthKit] An error happened when setting "
                                 @"up background observer - %@",
                                 error.localizedDescription);
  ...
  HKObserverQuery *query = [[HKObserverQuery alloc]
      initWithSampleType:sampleType
           ...
           updateHandler:^(HKObserverQuery *query,
             ...
             NSString *successEvent =
                 [NSString stringWithFormat:@"healthKit:%@:new", type];
             NSString *failureEvent =
                 [NSString stringWithFormat:@"healthKit:%@:failure", type];
  ...
  [self.healthStore
      enableBackgroundDeliveryForType:sampleType
                       ...
                       withCompletion:^(BOOL success,
                                        NSError *_Nullable error) {
                         NSString *successEvent = [NSString
                             stringWithFormat:@"healthKit:%@:setup:success",
                                              type];
                         NSString *failureEvent = [NSString
                             stringWithFormat:@"healthKit:%@:setup:failure",
                                              type];
                         ...
                         if (error) {
                           NSLog(@"[HealthKit] An error happened when setting "
                                 ...
                                 error.localizedDescription);
                           if (self.hasListeners) {
                             [self emitEventWithName:failureEvent
                                          andPayload:@{}];
                           }
                           return;
                         }
                         NSLog(
                             @"[HealthKit] Background delivery enabled for %@",
                             type);
                         [self.healthStore executeQuery:query];
                         if (self.hasListeners) {
                           NSLog(
                               @"[HealthKit] Background observer set up for %@",
                               type);
                           [self emitEventWithName:successEvent andPayload:@{}];
                         }
                       }];
...
- (void)fetchActivitySummary:(NSDate *)startDate
  ...
  NSPredicate *predicate = [HKQuery
      predicateForActivitySummariesBetweenStartDateComponents:startComponent
                                            endDateComponents:endComponent];

  HKActivitySummaryQuery *query = [[HKActivitySummaryQuery alloc]
      initWithPredicate:predicate
         resultsHandler:^(HKActivitySummaryQuery *query, NSArray *results,
                          NSError *error) {
           if (error) {
             // Perform proper error handling here
             NSLog(@"*** An error occurred while fetching the summary: %@ ***",
                   error.localizedDescription);
           ...
           dispatch_async(
               dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                 for (HKActivitySummary *summary in results) {
                   int aebVal = [summary.activeEnergyBurned
                       doubleValueForUnit:[HKUnit kilocalorieUnit]];
                   int aebgVal = [summary.activeEnergyBurnedGoal
                       doubleValueForUnit:[HKUnit kilocalorieUnit]];
                   int aetVal = [summary.appleExerciseTime
                       doubleValueForUnit:[HKUnit minuteUnit]];
                   int aetgVal = [summary.appleExerciseTimeGoal
                       doubleValueForUnit:[HKUnit minuteUnit]];
                   int ashVal = [summary.appleStandHours
                       doubleValueForUnit:[HKUnit countUnit]];
                   int ashgVal = [summary.appleStandHoursGoal
                       doubleValueForUnit:[HKUnit countUnit]];
...
- (void)fetchAttachmentForClinicalRecord:(NSString *)recordId
  ...
  if (!uuid) {
    NSError *error = [NSError
        errorWithDomain:@"com.healthkit.error"
                   code:0
               userInfo:@{
                 NSLocalizedDescriptionKey : @"Invalid record ID format"
               }];
    completion(nil, error);
    return;
  }

  // Use HKAttachmentStore to get attachments directly by UUID
  HKAttachmentStore *attachmentStore =
      [[HKAttachmentStore alloc] initWithHealthStore:self.healthStore];

  // First, we need to get the object by UUID
  HKSampleType *clinicalType = [HKObjectType
      clinicalTypeForIdentifier:HKClinicalTypeIdentifierClinicalNoteRecord];
  NSPredicate *predicate = [HKQuery predicateForObjectWithUUID:uuid];

  HKSampleQuery *query = [[HKSampleQuery alloc]
      initWithSampleType:clinicalType
               predicate:predicate
                   limit:1
         sortDescriptors:nil
          resultsHandler:^(HKSampleQuery *query, NSArray *results,
            ...
            HKSample *sample = [results firstObject];
            if (!sample) {
              // Try with a different clinical type if the first one fails
              HKSampleType *documentType =
                  [HKObjectType clinicalTypeForIdentifier:
                                    HKClinicalTypeIdentifierLabResultRecord];
              HKSampleQuery *fallbackQuery = [[HKSampleQuery alloc]
                  initWithSampleType:documentType
                           predicate:predicate
                               limit:1
                     sortDescriptors:nil
                      resultsHandler:^(HKSampleQuery *query, NSArray *results,
                        ...
                        if (error || results.count == 0) {
                          NSError *noSampleError =
                              [NSError errorWithDomain:@"com.healthkit.error"
                                                  code:1
                                              userInfo:@{
                          ...
                          completion(nil, noSampleError);
                          return;
                        }

                        [self processAttachmentForSample:results.firstObject
                                         attachmentStore:attachmentStore
                                         attachmentIndex:attachmentIndex
                                              completion:completion];
                      }];

              [self.healthStore executeQuery:fallbackQuery];
              return;
            }

            [self processAttachmentForSample:sample
                             attachmentStore:attachmentStore
                             attachmentIndex:attachmentIndex
                                  completion:completion];
          }];
...
- (void)processAttachmentForSample:(HKSample *)sample
    ...
    API_AVAILABLE(ios(16.0)) {
  [attachmentStore
      attachmentsForObject:sample
                completion:^(NSArray<HKAttachment *> *_Nullable attachments,
                             NSError *_Nullable error) {
                  if (error) {
                    completion(nil, error);
                    return;
                  ...
                  if (attachments.count == 0 ||
                    ...
                    NSError *noAttachmentsError =
                        [NSError errorWithDomain:@"com.healthkit.error"
                                            code:3
                                        userInfo:@{
                                          NSLocalizedDescriptionKey :
                                              @"No attachments found or index "
                                              @"out of range"
                                        }];
                  ...
                  HKAttachment *attachment = attachments[attachmentIndex];
                  HKAttachmentDataReader *dataReader =
                      [attachmentStore dataReaderForAttachment:attachment];

                  [dataReader readData:^(NSData *_Nullable data,
                                         NSError *_Nullable error) {
                    if (error) {
                      completion(nil, error);
                      return;
                    ...
                    // Convert data to base64 string for transmission to JS
                    ...
                    [response setObject:attachment.contentType
                                 forKey:@"contentType"];

                    if (attachment.name) {
                      [response setObject:attachment.name forKey:@"title"];
                    }

                    [response setObject:@(data.length) forKey:@"size"];

                    if (attachment.creationDate) {
                      [response setObject:[RCTAppleHealthKit
